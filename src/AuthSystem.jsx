...
function AuthModal({ onClose, onAuth, defaultFlow = "login" }) {
  ...
  /* ── GOOGLE MOCK ── */
  // Google flow: asks for Gmail, then signs in / registers with that email
  const handleGoogle = async () => {
    reset();
    if (!googleEmail.trim()) return setErr("Enter your Google (Gmail) address to continue.");
    if (!googleEmail.includes("@")) return setErr("Enter a valid email address.");
    setLoading(true);
    await new Promise(r => setTimeout(r, 900));
    const gEmail = googleEmail.trim().toLowerCase();
    const users = getUsers();
    let user = users[gEmail];
    if (!user) {
      // First-time Google sign-up: create account from Google email
      const gName = gEmail.split("@")[0].replace(/[._]/g, " ").replace(/\b\w/g, c => c.toUpperCase());
      user = {
        id: uid(), name: gName, email: gEmail, org: null,
        createdAt: now(), avatar: gName[0].toUpperCase(), pwHash: "", provider: "google",
      };
      users[gEmail] = user;
      saveUsers(users);
      addNotif(user.id, { title: "Google Account Created", body: `Welcome! Your account has been created with Google sign-in for ${gEmail}.`, type: "success" });
    } else {
      // Returning user — restore their stored profile
      addNotif(user.id, { title: "Google Sign-In", body: `Welcome back, ${user.name}! Signed in with Google.`, type: "info" });
    }
    const session = { id: user.id, name: user.name, email: user.email, org: user.org, avatar: user.avatar, token: uid(), provider: "google" };
    saveSession(session);
    setLoading(false);
    onAuth(session);
  };
            ...
            {showGoogleInput ? (
              <div style={{ background: "#0a0d14", border: `1px solid ${C.border}`, borderRadius: 8, padding: "14px 16px", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: C.text, fontFamily: "'DM Mono',monospace" }}>Sign in with Google</span>
                  <button onClick={() => { setShowGoogleInput(false); reset(); }} style={{ marginLeft: "auto", background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 14, padding: 0 }}>✕</button>
                </div>
                <div style={{ fontSize: 10, color: C.muted, fontFamily: "'DM Mono',monospace", marginBottom: 8 }}>
                  Enter your Gmail address. If you have an existing account with this email it will be restored.
                </div>
          ...
          <>
            {showGoogleInput ? (
              <div style={{ background: "#0a0d14", border: `1px solid ${C.border}`, borderRadius: 8, padding: "14px 16px", marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, color: C.muted, fontFamily: "'DM Mono',monospace", marginBottom: 8 }}>
                  Enter your Gmail address to create an account. Returning users will be signed in automatically.
                </div>
                <Input value={googleEmail} onChange={setGoogleEmail} placeholder="yourname@gmail.com" autoFocus type="email" />
                <Btn variant="google" fullWidth onClick={handleGoogle} loading={loading}>
                  Sign up with Google →
...
/* ═══════════════════════════════════════════════════════════════
   AUTH GATE — intercepts the click BEFORE the child fires it.
   Strategy: wrap child in a relative container, overlay a
   transparent capture div when user is NOT logged in. That div
   catches the click first (before the real button) and shows the
   auth modal instead. When user IS logged in, the overlay is
   removed so the real button works normally.
   ═══════════════════════════════════════════════════════════════ */
function AuthGate({ user, onAuth, children }) {
  const [showModal, setShowModal] = useState(false);
  // Store the child's onClick so we can fire it after login
  const pendingCb = useRef(null);

  const handleCapture = (e) => {
    e.stopPropagation();
    e.preventDefault();
    // Save child click handler to invoke after auth
  ...
  const handleAuthSuccess = (session) => {
    setShowModal(false);
    onAuth(session);
    // Fire the original action after state settles
  ...
  return (
    <>
      <div style={{ position: "relative", display: "inline-flex" }}>
        {/* The real child button — always rendered */}
        {children}
        {/* Invisible capture overlay — only when NOT logged in */}
        {!user && (
          <div
